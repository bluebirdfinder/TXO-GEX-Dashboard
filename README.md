# 📈 TAIFEX TXO GEX 動態儀表板 v5.0.0 🚀
> **台指期權 Gamma Exposure 波動度、三大法人與大戶 3 日籌碼矩陣、全量 270 檔個股期貨量化分析系統**
...
### 2. 📋 露米連續 3 日法人與大戶籌碼矩陣 (Rumi 3-Day History Matrix)
* **期貨未平倉動向表**：追蹤前五大、前十大、**前五特法**、**前十特法**、外資、投信、自營商連續 3 日部位變化（多單加碼、空翻多、強多等）。
* **現貨與選擇權全景表**：涵蓋三大法人現貨買賣超（外資/投信/自營買賣金額）、選擇權組合（BC/BP/SC/SP）與 **🎯 結算 OP 方向智慧預估**。
...
### 4. 💎 期交所全量 270 檔個股與 ETF 期貨篩選器 (270 Stock Futures)
...
### 5. ⚡ 3 秒極速現價連線與跳動時間標註 (3-Sec Real-Time Quotes & Live Timestamp)
* **Cloudflare Worker 免費代理**：每 3 秒自動抓取台指現價 ($S$)，即時重算 Black-Scholes Gamma ($\Gamma$)。
* **動態時間標記**：頂部現價卡片下方實裝 `⏱️ 報價跳動時間: 19:45:12`，掌握最新滴答連線狀態。

### 6. 🔴 台灣股市色彩規範與 FinTech 微光膠囊 (Taiwan Red/Green & Glowing Badges)
* 🔴 **紅漲 / Bull (多頭)**：`#ff5252`，代表上漲、多單加碼、Call GEX 買權牆。
* 🟢 **綠跌 / Bear (空頭)**：`#00e676`，代表下跌、空單加碼、Put GEX 賣權牆。
* 💎 **微光膠囊 (Glowing Badges)**：採用帶有微透明背景與邊框發光的 `▲ 強多` 膠囊標籤，呈現 TradingView 高級終端質感。
...
```text
txo-gex-dashboard/
├── index.html                 # 主頁面 HTML5（含雙籌碼矩陣、GEX切換、270檔篩選器）
├── style.css                  # CSS3 視覺系統（台灣紅綠配色、 FinTech 膠囊、深灰捲軸）
├── app.js                     # 前端邏輯（3秒報價連線、Plotly圖表渲染、解密模組）
├── taifex_catalog.json        # 期交所官方 270 檔個股/ETF 期貨完整目錄
├── README.md                  # 本系統說明文件 v5.0.0
