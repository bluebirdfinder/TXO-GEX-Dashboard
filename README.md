# 🐦 尋鳥 Bluebird Finder — TXO GEX 量化系統 (v63.3)
...
[![引擎版本](https://img.shields.io/badge/Engine-v63.3-ffd700?style=flat&logo=python)](scripts/fetch_and_calc_vision.py)

## 🌟 v62.4 尋鳥戰情室3個Critical bug修復 ✕ ADR真實報價 ✕ 選股快取97%覆蓋率

### 🔴 0. 尋鳥戰情室 3 個 Critical bug 修復
- **假OHLC面板**：TXF/TAIEX/OTC漲跌改真實數據，開盤/最高/最低無真實來源時誠實顯示「—」。
- **ADX Pro V3背離**：改用真實波段極值相對比較，不再綁定固定價位。
- **選股雷達**：改接真實 `screener_cache.json`，不再用雜湊值捏造訊號。
- **附帶**：ADR連動bug修正+真實Yahoo Finance報價；散戶籌碼日增減改真實快照比對；選股快取覆蓋率32%→97%（改用TWSE單日批量端點）。
...
## 🌟 v50.5 台指選擇權造市商 21 章量化實戰手冊與 Covered Call 完全體

### 🦅 1. 21 大章節造市商量化操盤手冊 (`docs/OPTIONS_QUANT_PLAYBOOK.md` & `index.html`)
- **全維度覆蓋**：全面收錄 GEX 護盤牆、VEX 偏斜、VIX 2σ 預期步長、200 點階梯金字塔、富邦犀利 2.0 組合單連續 IOC 讓點洗價單、一分鐘百點斷路器、死士區 vs 主力區雙軌人機分工調倉 SOP。
- **高低波動態變形**：收錄 VIX < 13 極端低波三大變形（延後閃電戰、不對稱 Call 側進攻、跨期日曆保護）與 VIX > 18~22+ 高隱波反手倒貨三步收割法。
- **Covered Call 4 大機構級變形**：PMCC 窮人掩護性買權、動態 Delta 避險（暴跌自動砍微台轉 Delta 0）、Covered Straddle 恐慌雙收流（利潤 2.5 倍）、Ratio 慢牛流。
- **100% 現貨本位不開槓桿週週收息**：戶頭存足 231 萬（46,230 點 × 50 元），週三/週一雙節奏收息，拆單 2:3 階梯防現貨被廉價收購，永無追繳斷頭風險！

### 🤖 2. 方案 B：全自動對接期交所與國際總經日曆之週報生成引擎 (`scripts/fetch_and_calc_vision.py`)
- **零人工介入自動排程更新**：實作 `generate_dynamic_weekly_focus(curr_twd)` 演算法，根據目前日期動態界定每週一至週五交易時段，自動識別本週重大總經事件。
...
### 📖 2. GEX ✕ VEX ✕ VIX 三位一體量化實戰指南與對策手冊
- **完整理論與實戰融合**：於 Card 9 的 `📖 VIX 教學` 彈窗中，全面收錄 GEX（價格減震/放大器）、VEX（波動率調倉海嘯）、VIX（表面風向觀測）鐵三角架構。
- **台灣市場獨特生態**：深度解析自營商 Delta 中性造市、週三結算日 Gamma Squeeze 尾盤急拉急殺，以及外資夜盤流動性斷層與 VEX 超跌機制。
- **四大共振情境矩陣**：完整整理雙正共振（金髮女孩收租）、雙負共振（閃崩斷頭海嘯）、地雷背離（暴風雨前寧靜）、恐慌極致反折等實戰策略。

### 📌 2. 日夜盤微觀結構速報：整合 VIX 實時恐慌警報與選擇權策略
- **即時恐慌位移**：於「日夜盤微觀結構速報」模組中加入 VIX 實時恐慌指標評級警報，依據目前恐慌指數與 Gamma 區間給出買賣方具體防守建議（例如：`恐慌爆發，選擇權賣方拉寬翅膀防守，買方關注轉折打腳`）。

### 🛠️ 3. 網頁 HTML DOM 結構修復與 0ms 秒開優化
- **消除全螢幕黑屏**：修復 `index.html` 中未閉合之彈窗標籤，並將所有 Modal 遮罩改為預設隱藏，徹底解決全螢幕黑罩遮擋問題。

### 📅 1. 富邦期貨本週市場焦點 (Fubon Weekly Market Focus 2026.08.31 - 09.04)
- **事件對應熱門股期**：完整注入富邦期貨本週市場焦點，包含 8/31 MSCI 季度調整（載板/記憶體股期）、9/01 ISM 製造業（MNQ/MES）、9/02 半導體展 + Dell 財報（設備/AI伺服器股期）、9/03 ISM 非製造業 + Broadcom/HPE 財報（MNQ/ASIC股期）、9/04 美國非農就業（MNQ/MES）。
- **避險雷達區塊渲染**：頂部避險雷達動態渲染本週焦點事件卡片與標的符號，方便交易者一目瞭然全週關鍵催化劑。
...
- **12 種二維共振實戰矩陣 (空間幾何 ✕ 籌碼強度 Level)**：
  - 徹底解決「大跌行情卻死板判定多頭軋空」的認知矛盾，在 UI Badge、Tooltip 與 Modal 診斷表格中提供 12 種全情境精確 SOP 指引。
