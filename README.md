...
### 🚨 1. 實時避險倒數雷達與 3 階風險視窗燈號 (`#macro-events-radar-panel`)
...
- **秒級動態計時器與動態樣式**：
  - **動態倒數**：即時計算 `⏱️ XX天 XX時 XX分 XX秒`。
  - **三重燈號與風暴視窗**：
    - `🟢 平穩觀察期 (> 12小時)`：GEX 牆位正常提供抑波與防守機能。
    - `🟡 變盤前夕警戒期 (2~12小時)`：IV 攀升、買方權利金變貴，防範假突破。
    - `🚨 衝擊告急風暴圈 (< 2小時)`：全框自動紅光脈衝閃爍，警示做市商對沖調整與 IV 劇烈抽離。
- **接續 5 大重磅總經日曆矩陣**：一目了然預覽一週內重磅催化劑與 GEX 避險操作指引。
