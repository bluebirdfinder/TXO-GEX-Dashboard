...
| # | 功能 | 說明 |
|---|---|---|
| 0 | **🛠️ 尋鳥戰情室3個Critical bug修復 ✕ ADR真實報價 ✕ 選股快取97%覆蓋率 (v63.4)** | 修復假OHLC面板、ADX背離寫死絕對價位、選股雷達雜湊碼假訊號；ADR_MAPPING變數作用域bug+改真實Yahoo Finance報價；散戶籌碼daily_change/prev_ratio/broker_snapshot改真實日對日快照比對；選股快取改用TWSE單日批量端點，覆蓋率32%→97%、執行時間20-30分鐘→15秒；新增 `DASHBOARD_DATA_SOURCE_MAP.md` 前台區塊×策略×數據來源持久對照表 |
