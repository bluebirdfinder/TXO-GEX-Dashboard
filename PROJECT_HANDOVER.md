# 🏛️ TXO GEX 儀表板 — 專案交接手冊 (v63.1)

本手冊記錄專案現狀、核心功能清單、數據引擎 Self-Audit 熱備援架構、開發 SOP 與個人筆電續接步驟。

---

## 📌 一、v63.1 完整功能清單與數據引擎架構

### 核心分析與視覺化模組

| # | 功能 | 說明 |
|---|---|---|
| 0 | **🛠️ 尋鳥戰情室3個Critical bug修復 ✕ ADR真實報價 ✕ 選股快取97%覆蓋率 (v63.1)** | 修復假OHLC面板、ADX背離寫死絕對價位、選股雷達雜湊碼假訊號；ADR_MAPPING變數作用域bug+改真實Yahoo Finance報價；散戶籌碼daily_change/prev_ratio/broker_snapshot改真實日對日快照比對；選股快取改用TWSE單日批量端點，覆蓋率32%→97%、執行時間20-30分鐘→15秒；新增 `DASHBOARD_DATA_SOURCE_MAP.md` 前台區塊×策略×數據來源持久對照表 |
