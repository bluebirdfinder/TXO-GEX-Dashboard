## 🔄 2026-09-16 早上交接：v63.3 已完成修復並本機發版驗證，push 前待使用者確認（取代下方兩個舊交接區塊的「待辦」部分，皆保留當歷史記錄）

**現況**：使用者晚上 ~02:00 交代要照上一輪交接文件的優先順序繼續處理，說「你自己安排優先順序，我大約早上 7 點半左右會起來」就去睡了。凌晨先完成清單第1、2優先項目並發版到v63.2；使用者早上起床後，用我在v63.2釋出後PDF核對時標記為「不確定」的「大額交易人十大合計/特定法人」項目，親自截圖TAIFEX官方原始頁面，結果**直接揪出一個從這支函式寫出來就存在的計算公式錯誤**（不是顯示問題，是真的算錯），已修復並發版到v63.3。**push 屬於外部可見動作，使用者稍早選擇「先看過diff再說」，所以目前還是只做到本機commit，還沒push**。新session或使用者接手時，第一件事是確認能否push。

**本次(v63.2→v63.3)完成的三項修復（皆已跑過 `python scripts/fetch_and_calc_vision.py` 全流程 + 瀏覽器實測驗證）**：
...
3. **🔴🔴 `fetch_official_taifex_large_trader()` 前五大/前十大交易人淨部位計算公式錯誤修正（v63.3，本次交接最重大發現）**：官方表格欄位是「買方前五大/買方前十大/賣方前五大/賣方前十大/全市場未沖銷」，正確淨部位＝同排名層級「買方－賣方」，但原程式碼誤用「買方前五大－買方前十大」在計算，根本不是多空淨部位。已修正欄位索引，並用TAIFEX官方「指定日期查詢」功能回補9/11、9/14兩天已被污染的歷史快照，不只修「未來」。驗證：9/15數字（近月/遠月/全月共6組）跟使用者提供的台新期貨PDF截圖逐位完全吻合。
...
| 🟡 中 | 4 | `fetch_official_taifex_specific_traders()`重用邏輯double-check | 這支函式重用`fetch_official_taifex_large_trader()`的`top5_spec_net`/`top10_spec_net`/`foreign`來產生「外資特法背離診斷」文字（見v63.0條目）。既然v63.3證實該函式的欄位索引曾經錯過，這支下游函式過去產生的文字診斷可能也連帶受影響，優先度從🟢提升到🟡，建議下次順便驗證這支函式的文字輸出現在（v63.3修復後）是否合理。 |
...
| 🟡 中 | 7 | `largeTraderOptQry`（選擇權大額交易人未沖銷部位結構表）目前完全沒有被使用 | 使用者早上詢問這份報表能不能用，確認我們程式碼目前沒有任何地方抓取或解析它。構想：可以用來印證Put Wall/Call Wall防守強度（若大額交易人在該履約價附近大量淨空賣權/買權，代表有真實大資金在防守，比單純看未平倉量更有說服力）。屬於新功能構想，不是bug，需要使用者確認想不想做再排入。 |

### ⏳ 還沒做、新session請按這個順序處理

| 優先度 | # | 項目 | 細節 |
|---|---|---|---|
| 🔴 最高 | 0 | **確認 v63.3 是否可以 push** | 這個session本機已經commit了三項修復（含v63.3最重大的大額交易人公式修正）並發版，但push是外部可見動作，需要使用者當面確認（使用者已表示要先看過diff）。第一件事：確認要不要push（`git log --oneline -5` 看是否還是本機領先，`git status`確認乾淨），使用者同意才能push。 |
