...
4. **完成 `room.html`/`room.css` 完整版面實測**（優先順序清單原本排第3項，稽核清單裡標記「從未實際開瀏覽器跑過畫面」的項目，這次終於系統性測完）：桌面版+手機版(375px)雙寬度測試，抽屜收合/展開、5個分頁時間週期切換、Sub-Chart 4四個子分頁(ADX/AO/CVD/大戶散戶動能)、指標設定Modal、AI軍師面板皆無橫向溢出、無跑版、全頁文字掃描無殘留null/undefined/NaN。**過程中發現並修好一個真bug**：GEX五大防線「VEX Early」價位標籤因為JS浮點數減法沒有四捨五入，畫面上直接顯示`45558.700000000004`這種原始浮點誤差，已修正（`Math.round(...*10)/10`）。大戶散戶動能面板確認正確顯示誠實的「無資料」狀態（等待另一session的富邦即時串接），不是假數據。
