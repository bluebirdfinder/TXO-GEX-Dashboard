...
| 優先度 | # | 項目 | 細節 |
|---|---|---|---|
| 🔴 最高 | 0 | **確認 v63.2 是否可以 push** | 這個session本機已經commit了#10、#11兩項修復並發版到v63.2，但push是外部可見動作，使用者睡覺中無法確認，所以卡在本機。新session/使用者起床後第一件事：確認要不要push（`git log --oneline -3` 看是否還是本機領先，`git status`確認乾淨），若使用者同意才能push。 |
| 🟡 中 | 1 | 兩份券商PDF尚未系統性逐頁核對完 | `【富邦期貨】國內盤後期權日報_2026.09.15.pdf`（11頁，完全沒讀過）、`台新期貨盤後日報20260915.pdf`（20頁，只用`pdftotext`粗讀部分內容抓到散戶比bug）。這是使用者最原始的請求（「幫我檢查我網頁是否正確」），只完成一部分。`pdftotext`（不加`-layout`）在這台機器正常，`pdftoppm`不行。 |
