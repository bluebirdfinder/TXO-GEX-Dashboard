## 🔄 2026-09-16 交接：上一個 session 已把 v63.1 部署上線，接手前請先讀這裡（取代下方 09-15 交接區塊的「待辦」部分，09-15 區塊保留當歷史記錄）

**現況**：`main` 與 `claude/bold-galileo-dy1oxb` 兩個分支已完全同步（都在 commit `451eedd`），`v63.1` 已經是線上真正在跑的版本（`ENGINE_VERSION`/`engine_version` 都確認是 `v63.1`）。09-15 深夜那批「散戶多空比分母修正、真實FOMC/四巫日日曆、VIX歷史回補、法人籌碼歷史重複bug修復」不只 commit 完成，**也已經正式發版（v63.1）並 push+merge 進 main**，`STATUS.md`/`HISTORY.md` 都已同步更新。

**這次交接新增的規則（使用者 2026-09-16 明確決定，已寫入 memory 與 `.claude/skills/release/SKILL.md`）**：以後完成一批會影響交易判斷的重大修復後，**AI 自己主動觸發發版流程（`release` skill），不用等使用者說「發版」**。只有小修改/文件調整不用每次都發版；不確定算不算「重大」時，優先問使用者而不是默默跳過發版。

### ✅ 已完成（依時間順序，累加自 09-15 深夜清單）
...
| 優先度 | # | 項目 | 細節 |
|---|---|---|---|
| 🔴 高 | 1 | 兩個5日矩陣表格「+0」應顯示「—」 | `populateInstitutionalMatrix()` 裡「期貨未平倉5日淨部位歷程」跟「現貨買賣超金額與選擇權金額5日歷程」兩張表，對沒有快照的日期用 `row.top5_net \|\| 0` 誤顯示「+0」（誤導成「法人零進出」而非「無資料」）。NIGHT那張表已經用 `cell()` helper 修過同類問題，這兩張還沒套用同樣修法。**使用者親自實測抓到，優先度最高**。 |
| 🟡 中 | 2 | DAY session 法人快照是否有跟NIGHT一樣的「借用值被誤存成永久歷史」風險 | NIGHT已修（`is_live`標記），DAY的 `lt_inst`/`fut_inst`/`opt_inst`/`stock_inst` 還沒加同樣的標記。目前沒有實際重複的證據（可能只是運氣好），但風險結構相同，建議比照補上。 |
| 🟡 中 | 3 | 兩份券商PDF尚未系統性逐頁核對完 | `【富邦期貨】國內盤後期權日報_2026.09.15.pdf`（11頁，完全沒讀過）、`台新期貨盤後日報20260915.pdf`（20頁，只用`pdftotext`粗讀部分內容抓到散戶比bug）。這是使用者最原始的請求（「幫我檢查我網頁是否正確」），只完成一部分。`pdftotext`（不加`-layout`）在這台機器正常，`pdftoppm`不行。 |
| 🟢 低 | 4 | TPEx OTC指數(`^TWOII`)歷史回補卡住 | Yahoo Finance歷史K棒API對這檔回傳空值，原因不明。需要找TPEx官方歷史指數端點替代。 |
| 🟢 低 | 5 | `futDailyMarketReport`編碼問題是否影響其他解析點 | 這個TAIFEX端點的中文標籤（合計/小計）用big5/cp950都無法正確解碼，`parse_taifex_fut_oi()`已改用結構特徵判斷解決，但如果程式裡還有其他地方文字比對同一端點的中文標籤，會有一樣的問題，還沒全面搜過。 |
| 🟢 低 | 6 | `fetch_official_taifex_specific_traders()`重用邏輯double-check | 目前判斷應該沒事（重用的是已經正確解析過的資料），但還沒100%驗證過。 |

### 🗣️ 對話情境／使用者溝通風格提醒（延續給新session）

使用者會主動用真實外部資料（券商App截圖、官方PDF報告）交叉驗證儀表板數字，發現不對會直接問——這是最有效的bug發現方式（目前為止最大的bug都是這樣抓到的），新session收到截圖/PDF時應優先當成最高優先度真實性驗證任務，直接查程式碼比對。使用者是外商食品法規背景，對台灣法規敏感，任何操作若可能觸及台灣法規要主動提醒。**進度彙報請用表格呈現，不要用巢狀條列**；有真正需要使用者決定的分岔點，用 AskUserQuestion 提出結構化選項，不要只寫在段落裡。**完成一批會影響交易判斷的重大修復後，主動觸發發版流程，不用等使用者說「發版」**——但不確定算不算「重大」時要先問。
