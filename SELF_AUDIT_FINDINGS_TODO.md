## 🔄 2026-09-16 凌晨交接：v63.2 已完成修復並本機發版驗證，push 前待使用者確認（取代下方兩個舊交接區塊的「待辦」部分，皆保留當歷史記錄）

**現況**：使用者晚上 ~02:00 交代要照上一輪交接文件的優先順序繼續處理，並說「你自己安排優先順序，我大約早上 7 點半左右會起來」就去睡了。這個 session 接著把清單第 1、2 優先項目都做完、實測、跑完 `bump_version.py` 發版到 `v63.2`、補寫了 HISTORY.md/STATUS.md，**但 push 屬於外部可見動作，使用者不在場無法當面確認，所以只做到本機 commit，還沒 push**。新 session（或使用者本人早上起來後）接手時，第一件事應該是跟使用者確認可以 push 了沒有。

**本次完成的兩項修復（已跑過 `python scripts/fetch_and_calc_vision.py` 全流程 + 瀏覽器實測驗證）**：
1. `populateInstitutionalMatrix()`（`app.js`）「期貨未平倉5日歷程」「現貨與選擇權5日歷程」兩張表格的「+0」→「—」修正（含連帶發現的 `pc_ratio` 借用今天數字冒充過去日期的問題）。
2. DAY session 法人快照（`fetch_official_taifex_large_trader`/`fetch_official_taifex_futures_institutional_oi`/`fetch_official_taifex_options_matrix`/`fetch_twse_institutional_stock_trading` 這4支函式）補上 `is_live` 標記，比照9/15修好的NIGHT session防呆機制，避免借用舊值被誤存成永久歷史。

詳細技術內容見 [HISTORY.md](HISTORY.md) `v63.2` 條目，不在這裡重複。
...
| 大額交易人top5/top10淨部位（全月） | top5 -9,826／top10 -23,510 | 即時官方端點直接重抓比對，一致 | ✅ 一致（未找到第三方PDF對照值） |
...
| 🟢 低 | 1 | 大額交易人「十大合計／特定法人」細節分類，跟台新PDF頁7側欄數字對不太起來，但無法確定是真的不同還是我看錯圖表軸標籤 | 台新PDF頁7有「十大合計 近月-3052／遠月6558／全月3506」「特定法人 近月-7900／遠月7115／全月-785」這組數字，跟我們`fetch_official_taifex_large_trader()`算出的near/far/total top10_net（-3996/-19514/-23510）完全對不起來，數量級與正負號都不同。**因為這台機器沒有pdftoppm，只能看PyMuPDF抓出來的純文字，看不到圖表實際排版，無法100%確定「十大合計」這個標籤實際對應的是哪個TAIFEX官方表格欄位**（可能是不同的分類口徑，例如「大額交易人未沖銷部位」報表本身有「前五大/前十大」交易人 × 「一般/特定法人」多層次分類，我們可能漏抓或誤配了其中一層）。這個不確定性比投信現貨那筆更大，且涉及大額交易人籌碼判讀（實際會被使用者拿來做交易判斷），**建議之後裝了pdftoppm或有辦法看到PDF視覺版面後，专门花時間比對這一段，不要用文字猜**。 |
| 🟢 低 | 2 | TPEx OTC指數(`^TWOII`)歷史回補卡住 | Yahoo Finance歷史K棒API對這檔回傳空值，原因不明。需要找TPEx官方歷史指數端點替代。 |
...
| 🟢 低 | 4 | `fetch_official_taifex_specific_traders()`重用邏輯double-check | 目前判斷應該沒事（重用的是已經正確解析過的資料），但還沒100%驗證過。 |
...
| 🟢 低 | 6 | 台新PDF還有頁12~25尚未看完（本次看到頁11為止），富邦PDF頁1/7為空頁/免責聲明，已確認無漏看內容。 |

### ⏳ 還沒做、新session請按這個順序處理

| 優先度 | # | 項目 | 細節 |
|---|---|---|---|
| 🔴 最高 | 0 | **確認 v63.2 是否可以 push** | 這個session本機已經commit了兩項修復並發版到v63.2，但push是外部可見動作，使用者睡覺中無法確認，所以卡在本機。新session/使用者起床後第一件事：確認要不要push（`git log --oneline -3` 看是否還是本機領先，`git status`確認乾淨），若使用者同意才能push。 |
