...
**已知殘留、記錄但這次沒修**：
- `trading room/room.js` 內至少發現4組不同函式各自對`call_wall_strike`/`zero_gamma_level`/`put_wall_strike`/`max_pain_strike`寫了不同的保底字面值（例如cw的保底值在不同函式裡分別是`47400`/`46400`/`47300`），跟昨晚在app.js發現並清理的「34處不一致保底值」是同一種模式，只是這次在room.js。範圍比較大（初步看到至少4組×4個欄位=16處，可能更多），建議之後排時間用昨晚app.js CHART_DEFAULTS的做法比照處理。
- `renderLeftPanel()`裡另外還有幾處類似的個股報價保底值（`395.52`/`-9.72`/`46184.85`/`-755.64`等），同一類問題，一併列入上述後續清理範圍。
