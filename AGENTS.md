...
### 🔴 6. 嚴禁使用任何偽數據、數學隨機擬合波形或憑感覺畫圖（杜絕任何虛假幻覺）
- **原理**：使用者為實盤真金白銀交易。任何未經真實市場授權的偽數據（如布朗運動隨機擬合、正弦波模擬、假 Tick 跳動或未嚴格對齊 Pine Script 的簡化指標公式）均會導致操盤手誤判行情、引發巨大實盤損失，屬於重大災難級違規。
- **鐵律**：
  1. **行情與 K 線數據 100% 來自真實官方數據源**：
