...
### 5. 🏷️ 10 盤歷史演變動態狀態徽章 (Dynamic Session Badging)
- **三色狀態分級**：在 10 盤演變推進條上加入明確徽章分類 — 🔥 **紅字閃爍** (Live 即時跳動)、🟡 **金黃字體** (日盤收盤快照/待16:00清算)、🟣 **粉藍字體** (官方 100% 結算定案版)。
- **時序自動維護**：由 `scripts/fetch_and_calc_vision.py` 自動依據 UTC+8 時間視窗判斷，確保即時盤中與歷史盤後演變之嚴密連貫性。
