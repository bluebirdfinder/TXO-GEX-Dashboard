$jsonPath = Join-Path $PSScriptRoot "..\data\gex_data.json"
$outPath  = Join-Path $PSScriptRoot "..\data\encrypted_gex.json"
$keyString = "GEX2026"

$rawText = [System.IO.File]::ReadAllText($jsonPath, [System.Text.Encoding]::UTF8)
$data = $rawText | ConvertFrom-Json

# Update recent_3_days_summary with real numbers
$t0 = [ordered]@{
    date_label = "8/03 (T日)"
    spot_price = 43386.41
...
$summary = @($t0, $t1, $t2)
$data | Add-Member -NotePropertyName "recent_3_days_summary" -NotePropertyValue $summary -Force
...
Write-Host "Success: Updated and encrypted!"
