...
### 進行中 / 待辦
- [ ] `trades` 頻道訂閱（逐筆成交 + 口數分類，用於「散戶成交筆數差」）。
- [ ] 30 分鐘線聚合邏輯（大戶委託口差、散戶成交筆數差、市場委買委賣口差三條線）。
- [ ] 個股期貨（成交量前 10-20 大）範圍的 Books/Trades 訂閱擴充。
- [ ] 將 `room.js` 現有的假動能公式（room.js:702-734）換成串接上述真實資料。
- [ ] 2026-09-14 台指期 08:45 開盤：實測 Books 頻道是否真的收到資料（確認富邦帳號有無委託簿資料權限）。
